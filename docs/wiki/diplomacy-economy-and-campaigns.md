...
These are future gameplay domains. v0.4.0 supplies a typed hail and bounded ship behavior, not treaties, faction planning, trade, governments, or a generated campaign. The approved political structure is described separately from the implementation scope of the next slice.

## Living sector and faction autonomy

M5 is intended to prove faction intent → autonomous ship assignment → offscreen activity → durable world change. The roadmap proposes roughly three to five locations, two factions, and several NPC ships, with at least one NPC-NPC interaction that matters without the player witnessing it.

Faction decisions should use available information, goals, doctrine, constraints, and a bounded resource or capability. They should issue existing ship orders where appropriate, wake at meaningful simulation boundaries, and produce explanations. The scenario must demonstrate a choice that changes with circumstances, not merely replay a fixed patrol script.

A noncombat interaction can prove the chain before M6. Reports, observed contacts, investigation, rerouting, withdrawal, or support are candidate mechanisms, not all required features. Long-horizon tests should catch starvation, zero-time loops, order oscillation, unbounded state growth, dangling references, and save/load divergence.

The layered political model does not require implementing every depth, organization type, government mechanism, or covert operation in M5. The governing feature must choose the smallest complete consumer of those principles.
...
[Roadmap M5-M9](../../ROADMAP.md), [political framework](factions-and-organizations.md), [narrative ADR](../adr/0012-keep-branching-narrative-subordinate-to-simulation.md), [persistence ADR](../adr/0006-use-versioned-json-snapshot-saves.md), and [original owner brief and archived discussion](../design/initial-brainstorm-session.md).
