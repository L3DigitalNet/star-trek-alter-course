...
D-11's implementation preserves existing ship-work meaning, total same-time sequence, exact correlation, typed target validation, and budgets. Faction decisions wake initially, at arrival, or at a future hold/travel release boundary; they do not poll at tactical frequency. A pending objective is dormant only when it has no eligible candidate and no release boundary. Typed bootstrap creates a complete valid aggregate, including faction state and initial work. It schedules the ship's existing work before the faction's decision work, so a wake observes the already-established order/travel state instead of a partially initialized world.
