...
All six decisions below are **owner-approved design, not implemented**. [Faction Intent and Autonomous Assignment](faction-intent-and-autonomous-assignment.md) owns their complete meaning and required proof. They select the first bounded contribution toward M5, not the entire milestone. Documentation approval does not start runtime implementation, advance the current V6 save schema, or publish a release.
...
**D-12 — Planned V7 and non-inventive migration.** Use ordinary validated JSON faction definitions and explicit mutable runtime state. The later implementation introduces V7 and an adjacent V6→V7 migration with zero factions, null historical ship controllers, and no faction state or wakes. Existing ship work retains its semantics and ordering as explicitly ship-targeted work. Zero-faction worlds remain valid; no new-game political history is injected into old saves.
