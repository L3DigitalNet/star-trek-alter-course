...
description: 'Records ADR 0003 evidence for AssetCtl schema, YAML, logging, raster, and SVG dependencies.'
doc_type: 'reference'
status: 'active'
created: '2026-09-01'
updated: '2026-09-01'
...
| --- | --- | --- | --- |
| `JsonSchema.Net` 9.4.0 | Draft 2020-12 meta-schema validation for tracked schema documents. `System.Text.Json` parses JSON but does not implement JSON Schema dialects or keyword contracts. | Current NuGet package; targets .NET 10, .NET 8, and .NET Standard 2.0; MIT; source is `json-everything`; transitively uses `JsonPointer.Net` 7.0.2 or later. | Unknown dialects and malformed schema keyword values fail configuration validation. Confined to `JsonSchemaDocumentValidator`; replace behind that boundary. |
...
Package provenance: [JsonSchema.Net](https://www.nuget.org/packages/JsonSchema.Net/9.4.0), [json-everything upstream](https://github.com/gregsdennis/json-everything), [YamlDotNet](https://www.nuget.org/packages/YamlDotNet/18.1.0), [Microsoft.Extensions.Logging](https://www.nuget.org/packages/Microsoft.Extensions.Logging/10.0.11), [Serilog](https://www.nuget.org/packages/Serilog/4.4.0), [SkiaSharp](https://www.nuget.org/packages/SkiaSharp/4.151.1), [SkiaSharp upstream](https://github.com/mono/SkiaSharp), and [Svg.Skia](https://www.nuget.org/packages/Svg.Skia/5.2.3).
