...
description: 'Defines the canonical format, validation pipeline, identity rules, and Godot boundary for data-driven game definitions.'
doc_type: 'adr'
status: 'active'
created: '2026-09-01'
updated: '2026-09-01'
...
This decision governs authored, generated, or imported definitions that describe reusable simulation content. It applies when a definition is intended to be loaded by `AlterCourse.Core`, referenced by stable identity, or validated as part of the game's content set.

It does not govern save games, runtime world state, visual assets, Godot scene files, Godot import metadata, transient caches, or tool configuration that has an independently justified native format. Save games are governed by ADR 0006.
...
- The project should avoid adding a custom authoring language or database.
- Future mod support must remain possible without being designed prematurely.

## Considered Options

- Canonical JSON with System.Text.Json, JSON Schema, and semantic validation.
...
Chosen option: "Canonical JSON with System.Text.Json, JSON Schema, and semantic validation", because it gives Core a strict, portable, toolable representation while supporting both structural and domain validation.

This decision governs reusable definitions consumed by Core. It does not require all definitions to exist before gameplay needs them, and it does not prescribe one monolithic schema or directory layout.

### Canonical format and parser

Canonical domain content is stored as UTF-8 JSON and deserialized with `System.Text.Json`.
...
A YAML file that defines ships, factions, weapons, world topology, or other Core content would require an amendment to this ADR. The project will not support parallel JSON and YAML representations for the same content family because dual representations create inconsistent validation and tooling.
...
Generated content is not trusted merely because a repository tool created it. It passes the same parse, schema, reference, and semantic validation as hand-authored content.
...
- Good, because domain content is readable, diffable, and independent from Godot.
...
- Good, because agents can generate content against explicit machine-readable constraints.
...
- Bad, because authors must understand the distinction between simulation and presentation records.
- Bad, because content changes can require explicit migration work instead of permissive fallback.

### Confirmation

A file is in scope when it defines reusable data consumed by Core or referenced by stable identity from simulation state.

Conformance is confirmed by:

- successful strict parsing and schema validation;
...
- headless loading from `AlterCourse.Core.Tests`;
- the absence of Godot dependencies from canonical definition models;
...
- Bad, because accepting both JSON and YAML would duplicate the content pipeline.
...
JSON Schema validates document structure, not all gameplay meaning. The semantic-validation stage is therefore part of the chosen option rather than an optional enhancement.

The canonical format may be presented through custom Godot editor tools later. Such tooling should edit or generate the JSON contract rather than silently establish a second source of truth.
