...
internal static class PromptCompiler
{
    ...
    public static (string Prompt, string Hash) Compile(AssetRequest request, StyleProfile style)
    {
        ...
        builder.AppendLine(
            ...
            $"Output: {request.Output.Format.ToString().ToLowerInvariant()} {request.Output.Width}x{request.Output.Height}; transparency {(request.Output.TransparencyRequired ? "required" : "optional")}"
        );
        builder.AppendLine(CultureInfo.InvariantCulture, $"Style: {style.Summary}");
        builder.AppendLine(
            CultureInfo.InvariantCulture,
            $"Required: {string.Join("; ", style.Required.Concat(request.Required))}"
        );
        builder.AppendLine(
            CultureInfo.InvariantCulture,
            $"Prohibited: {string.Join("; ", style.Prohibited.Concat(request.Prohibited))}"
        );
        builder.AppendLine(
            CultureInfo.InvariantCulture,
