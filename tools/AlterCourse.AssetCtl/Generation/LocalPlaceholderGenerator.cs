...
internal sealed class LocalPlaceholderGenerator : IAssetGenerator
{
    ...
    public Task<GenerationBatchResult> GenerateAsync(ProviderExecutionContext context, NormalizedGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        byte[] bytes = request.Request.Output.Format switch
        {
            ...
            _ => throw new ProviderException(ProviderErrorCategory.UnsupportedOutput, "Unsupported local output format."),
        };
    ...
    public static byte[] RenderSvg(AssetRequest request)
    {
        ...
        string svg = $"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"{width}\" height=\"{height}\" viewBox=\"0 0 {width} {height}\"><rect x=\"1\" y=\"1\" width=\"{width - 2}\" height=\"{height - 2}\" rx=\"{Math.Max(2, Math.Min(width, height) / 10)}\" fill=\"none\" stroke=\"#{color}\" stroke-width=\"2\"/><path d=\"M {width / 4} {height / 4} L {width * 3 / 4} {height * 3 / 4} M {width * 3 / 4} {height / 4} L {width / 4} {height * 3 / 4}\" stroke=\"#{color}\" stroke-width=\"{Math.Max(2, Math.Min(width, height) / 12)}\"/><title>Placeholder {System.Security.SecurityElement.Escape(request.Id)} {label}</title></svg>";
        return Encoding.UTF8.GetBytes(svg);
    }

    public static byte[] RenderPng(AssetRequest request)
    {
        var info = new SKImageInfo(request.Output.Width, request.Output.Height, SKColorType.Rgba8888, SKAlphaType.Premul);
        using var surface = SKSurface.Create(info);
        SKCanvas canvas = surface.Canvas;
        canvas.Clear(SKColors.Transparent);
        string hex = Color(request.Id);
        var color = SKColor.Parse('#' + hex);
        using var paint = new SKPaint { Color = color, IsAntialias = false, Style = SKPaintStyle.Stroke, StrokeWidth = Math.Max(2, Math.Min(info.Width, info.Height) / 12f) };
    ...
    private static string Initials(string id) => string.Concat(id.Split('.').TakeLast(2).Select(part => char.ToUpperInvariant(part[0])));
}
