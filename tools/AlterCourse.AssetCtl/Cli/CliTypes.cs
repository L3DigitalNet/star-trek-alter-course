...
internal static class CliTypes
{
    ...
    )
    {
        ...
        internal static object Doctor(EffectiveConfiguration configuration, CliOptions options)
        {
            ...
            {
                throw new AssetCtlException(
                    "Live provider probes are not implemented; this command never spends silently.",
                    2
                );
            }
            ...
            return new
            {
                ...
                providers = configuration
                    .Providers.Values.OrderBy(provider => provider.Id, StringComparer.Ordinal)
                    .Select(provider => new
                    {
                        id = provider.Id,
                        adapter = provider.AdapterId,
                        provider.Enabled,
                        credential = provider.CredentialEnvironmentVariable is null
                            ? null
                            : new
                            {
                                variable = provider.CredentialEnvironmentVariable,
                                present = !string.IsNullOrEmpty(
        ...
        private static object Find(EffectiveConfiguration configuration, CliOptions options)
        {
            ...
            object[] values = ManifestStore
                .LoadAll(configuration)
                .Where(manifest => id is null || string.Equals(manifest.Request.Id, id, StringComparison.Ordinal))
                .Where(manifest => kind is null || string.Equals(manifest.Request.Kind, kind, StringComparison.Ordinal))
                .Where(manifest =>
                    lifecycle is null
                    || string.Equals(
                        manifest.Request.Lifecycle.ToString(),
                        lifecycle,
                        StringComparison.OrdinalIgnoreCase
                    )
                )
                .Where(manifest => tag is null || manifest.Request.Tags.Contains(tag, StringComparer.OrdinalIgnoreCase))
                .Where(manifest =>
                    style is null || string.Equals(manifest.Request.StyleProfile, style, StringComparison.Ordinal)
                )
                .Where(manifest =>
                    query is null
                    || manifest.Request.Id.Contains(query, StringComparison.OrdinalIgnoreCase)
                    || manifest.Request.Purpose.Contains(query, StringComparison.OrdinalIgnoreCase)
                )
                .OrderBy(manifest => manifest.Request.Id, StringComparer.Ordinal)
                .Select(manifest =>
                    (object)
                        new
                        {
        ...
        private static object Status(EffectiveConfiguration configuration)
        {
            global::System.Collections.Generic.IReadOnlyList<global::AlterCourse.AssetCtl.Domain.DomainModels.AssetManifest> catalog =
                ManifestStore.LoadAll(configuration);
            int missing = catalog.Count(manifest =>
                !File.Exists(Path.Combine(configuration.RepositoryRoot, manifest.Request.Output.Path))
            );
            int mismatched = 0;
            foreach (
                global::AlterCourse.AssetCtl.Domain.DomainModels.AssetManifest manifest in catalog.Where(manifest =>
                    manifest.Integrity is not null
                    && File.Exists(Path.Combine(configuration.RepositoryRoot, manifest.Request.Output.Path))
                )
            )
            {
        ...
        )
        {
            ...
            {
                throw new AssetCtlException(
                    "Approval requires passing validation and complete non-placeholder rights data.",
                    8
                );
            }
        ...
        private static object Deprecate(EffectiveConfiguration configuration, CliOptions options)
        {
            ...
            {
                throw new AssetCtlException(
                    "Deprecating an approved asset requires exact --confirm-approved-asset.",
                    8
                );
            }
        ...
        private static string Capability(AssetCapability capability) =>
            capability switch
            {
                AssetCapability.RasterGenerate => "raster.generate",
                AssetCapability.VectorGenerate => "vector.generate",
                AssetCapability.ImageEdit => "image.edit",
                AssetCapability.ImageReferenceInput => "image.reference-input",
                AssetCapability.ImageTransparentOutput => "image.transparent-output",
                AssetCapability.ImageBackgroundRemove => "image.background-remove",
                AssetCapability.ImageVectorize => "image.vectorize",
                AssetCapability.ReviewSemantic => "review.semantic",
                AssetCapability.ReviewReferenceComparison => "review.reference-comparison",
    ...
    public static class RepositoryLocator
    {
        public static string Find(string start)
        {
            ...
            {
                if (
                    File.Exists(Path.Combine(directory.FullName, "AlterCourse.sln"))
                    && Directory.Exists(Path.Combine(directory.FullName, "config", "assets"))
                )
                {
