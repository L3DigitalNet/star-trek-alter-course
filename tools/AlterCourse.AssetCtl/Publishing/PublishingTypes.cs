...
public sealed class AssetLock : IDisposable
{
    ...
    {
        string stateRoot = PathPolicy.ResolveUnder(configuration.RepositoryRoot, configuration.Paths.StateRoot, "state_root", allowMissing: true);
        string lockRoot = Path.Combine(stateRoot, "locks");
        Directory.CreateDirectory(lockRoot);
        string safeId = string.Concat(assetId.Select(character => char.IsAsciiLetterOrDigit(character) ? character : '_'));
        ...
        JsonSerializer.Serialize(lockStream, new { process_id = Environment.ProcessId, acquired_at = DateTimeOffset.UtcNow });
...
public static class AtomicPublisher
{
    ...
    {
        string asset = PathPolicy.ResolveUnder(configuration.RepositoryRoot, assetRelativePath, "asset output", allowMissing: true);
        string manifest = PathPolicy.ResolveUnder(configuration.RepositoryRoot, manifestRelativePath, "manifest output", allowMissing: true);
...
public static class ReceiptWriter
{
    ...
    {
        string root = PathPolicy.ResolveUnder(configuration.RepositoryRoot, configuration.Paths.ReceiptRoot, "receipt_root", allowMissing: true);
        Directory.CreateDirectory(root);
        string id = Guid.NewGuid().ToString();
        string path = Path.Combine(root, id + ".json");
        string stage = path + ".tmp";
        File.WriteAllText(stage, JsonSerializer.Serialize(receipt, JsonOptions.Indented), new System.Text.UTF8Encoding(false));
