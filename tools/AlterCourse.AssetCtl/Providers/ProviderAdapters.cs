...
public sealed class RecraftImageAdapter(HttpClient client) : HttpProviderBase(client), IAssetGenerator
{
    ...
    public async Task<GenerationBatchResult> GenerateAsync(ProviderExecutionContext context, NormalizedGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            Content = JsonContent.Create(new { model = context.Model.VendorModel, prompt = request.Prompt, n = request.CandidateCount, response_format = "b64_json" }),
        };
        ...
        global::System.Collections.Generic.IReadOnlyList<global::AlterCourse.AssetCtl.Domain.DomainModels.GeneratedCandidate> candidates = await ParseImageDataAsync(response.RootElement, context, cancellationToken).ConfigureAwait(false);
    ...
    internal static void ValidateKnown(IReadOnlyDictionary<string, string> options, params string[] known)
    {
        ...
        {
            throw new ProviderException(ProviderErrorCategory.InvalidRequest, $"Unknown provider option '{unknown}'.");
        }
    }

    internal static string? RequestId(JsonElement root) => root.TryGetProperty("id", out JsonElement id) ? id.GetString() : null;
...
public sealed class OpenAiImageAdapter(HttpClient client) : HttpProviderBase(client), IAssetGenerator
{
    ...
    public void ValidateOptions(IReadOnlyDictionary<string, string> options) => RecraftImageAdapter.ValidateKnown(options, "quality", "background", "output_compression");

    public async Task<GenerationBatchResult> GenerateAsync(ProviderExecutionContext context, NormalizedGenerationRequest request, CancellationToken cancellationToken)
    {
        ValidateOptions(context.Model.Options);
        bool editing = request.References.Count != 0;
        using var message = new HttpRequestMessage(HttpMethod.Post, RecraftImageAdapter.Endpoint(context.Provider.Endpoint!, editing ? "images/edits" : "images/generations"));
        if (editing)
        {
            ...
            multipart.Add(new StringContent(request.CandidateCount.ToString(System.Globalization.CultureInfo.InvariantCulture)), "n");
        ...
        {
            message.Content = JsonContent.Create(new { model = context.Model.VendorModel, prompt = request.Prompt, n = request.CandidateCount, output_format = "png" });
        }

        using global::System.Text.Json.JsonDocument response = await SendJsonAsync(message, context, cancellationToken).ConfigureAwait(false);
        return new GenerationBatchResult(await ParseImageDataAsync(response.RootElement, context, cancellationToken).ConfigureAwait(false), RecraftImageAdapter.RequestId(response.RootElement), null);
    }
}
...
public sealed class XaiImageAdapter(HttpClient client) : HttpProviderBase(client), IAssetGenerator
{
    ...
    public void ValidateOptions(IReadOnlyDictionary<string, string> options) => RecraftImageAdapter.ValidateKnown(options, "aspect_ratio", "resolution");
    ...
    public async Task<GenerationBatchResult> GenerateAsync(ProviderExecutionContext context, NormalizedGenerationRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            payload["image"] = request.References.Select(reference => Convert.ToBase64String(reference.Bytes)).ToArray();
        }

        using var message = new HttpRequestMessage(HttpMethod.Post, RecraftImageAdapter.Endpoint(context.Provider.Endpoint!, "images/generations")) { Content = JsonContent.Create(payload) };
        using global::System.Text.Json.JsonDocument response = await SendJsonAsync(message, context, cancellationToken).ConfigureAwait(false);
        return new GenerationBatchResult(await ParseImageDataAsync(response.RootElement, context, cancellationToken).ConfigureAwait(false), RecraftImageAdapter.RequestId(response.RootElement), null);
    }
}
